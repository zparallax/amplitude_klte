...
BUILD_CROSS_COMPILE=../prebuilts/gcc/arm/linaro_4.9.3/bin/arm-eabi-
...
case $1 in
		...
		*)
		BUILD_KERNEL_OUT_DIR=$BUILD_TOP_DIR/okernel_tw
		PRODUCT_OUT=$BUILD_TOP_DIR/okernel_tw
...
FUNC_MKBOOTIMG()
{
	...
	echo "	$MKBOOTIMGTOOL --kernel $KERNEL_ZIMG \
			--ramdisk $PRODUCT_OUT/ramdisk.gz \
	...
	if ! [ -e $PRODUCT_OUT/out ] ; then
		mkdir -p $PRODUCT_OUT/out
	fi

	cd $PRODUCT_OUT/out
	
	rm ./amplitude_tw_${MODEL}_${CARRIER}_${CERTIFICATION}.zip
	
	cp ../boot.img .
	cp -R $BUILD_TOP_DIR/../recovery/META-INF .
	cp -R $BUILD_TOP_DIR/../recovery/system .
	
	#tar cvf boot_${MODEL}_${CARRIER}_${CERTIFICATION}.tar boot.img
	zip -r ./amplitude_tw_${MODEL}_${CARRIER}_${CERTIFICATION}.zip . || exit -1
        cd ~
